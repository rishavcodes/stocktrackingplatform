// Shared portfolio-pricing normalization.
//
...
// docs (and the back-compat `fees`/`feeValidity` mirrors we keep writing) are
// normalized through here so every read site sees one consistent shape and
...
export function feeValidityToDays(feeValidity?: string | null): number {
