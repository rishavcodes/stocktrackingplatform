...
import { Service } from "@/components/Marketplace/types";
...
async function fetchServices(id: string): Promise<Service[]> {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/services/allservices?id=${id}`,
      {
        method: "GET",
        headers: {
...
export default function ExpertServicesPage() {
  ...
  const segments = Array.from(
    new Set(serviceData.map((s) => s.segment).filter(Boolean))
  ) as string[];

  const filtered = serviceData.filter((s) => {
    if (segmentFilter !== "all") {
      if (s.segment !== segmentFilter) return false;
    }
    return true;
  });
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((service) => (
            <ServiceCard key={service._id} service={service} marketplaceSlug={slug} />
          ))}
        </div>
      )}
    </div>
