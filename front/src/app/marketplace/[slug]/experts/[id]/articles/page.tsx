  ...
  useEffect(() => {
    ...
    const loadArticles = async () => {
      ...
      try {
        ...
        // Use marketplace API endpoint - returns only public, marketplace-tagged articles
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/marketplace/${slug}?type=articles&page=1&limit=100`
        );
        ...
        if (isMounted && result.success && Array.isArray(result.data)) {
          ...
          const expertArticles = result.data.filter(
            (article: Article) => article.authorData?.id === id
          );
          setArticleData(expertArticles);
        }
      } catch (err) {
        if (isMounted) {
      ...
      <div className="flex justify-center items-center py-12">
        <div className="text-center">
          ...
          <p className="text-gray-500 dark:text-gray-500">
            There are no public articles available at the moment.
          </p>
        </div>
      </div>
    );
