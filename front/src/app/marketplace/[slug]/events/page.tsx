...
import { ArrowLeft, Filter, Search, CalendarDays } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Event, Marketplace } from "@/components/Marketplace/types";
import { EventCard } from "@/components/Marketplace/CardComponents";
...
export default function AllEventsPage() {
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/marketplace/${marketplaceId}?type=events&page=1&limit=100`
        );
  ...
  const eventTypes = Array.from(new Set(list.map((e) => e.eventType).filter(Boolean))) as string[];
  const categories = Array.from(
    new Set(list.flatMap((e) => (Array.isArray(e.category) ? e.category : e.category ? [e.category] : [])))
  ...
  const hasActiveFilters = searchQuery || eventTypeFilter !== "all" || categoryFilter !== "all";
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-wrap gap-3">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-medium text-slate-500 uppercase">Event type</span>
                  <select
