...
// The new simplified wizard doesn't ask for it — every new ticket goes in as
// "normal". Renders only when ≠ "normal", so unset values are invisible.
...
// Category → sub-category map, mirrored from the SP sidebar so tickets group
// around the exact surface where the issue happened. Adding a new sidebar
// tab? Add a row here.
type CategoryMeta = {
  label: string;
  Icon: typeof MessageSquare;
  color: string;     // text + bg classes for the icon tile
  subs: string[];
};

// Simplified 7-category tree — fewer choices, less cognitive load for RAs.
// Each category has only the sub-categories actually relevant to it.
const CATEGORY_TREE: Record<string, CategoryMeta> = {
  accounts:       { label: "Accounts",        Icon: WalletIcon,    color: "text-emerald-600 bg-emerald-50", subs: ["Wallet", "Charges", "Payment Gateway"] },
  content:        { label: "Content",         Icon: Newspaper,     color: "text-orange-600 bg-orange-50",   subs: ["Event", "Articles"] },
  modelportfolio: { label: "Model Portfolio", Icon: PieChart,      color: "text-cyan-600 bg-cyan-50",       subs: ["Other"] },
  course:         { label: "Course",          Icon: GraduationCap, color: "text-blue-600 bg-blue-50",       subs: ["Other"] },
  profile:         { label: "Profile",          Icon: GraduationCap, color: "text-blue-600 bg-blue-50",       subs: ["Other"] },
  recommendation:         { label: "Recommendation",          Icon: GraduationCap, color: "text-blue-600 bg-blue-50",       subs: ["Other"] },
  onboarding:     { label: "Onboarding",      Icon: UserPlus,      color: "text-purple-600 bg-purple-50",   subs: ["Plans", "Coupon", "Package"] },
  subscriber:     { label: "Subscriber",      Icon: Users,         color: "text-violet-600 bg-violet-50",   subs: ["Leads", "Subscriber"] },
  other:          { label: "Other",           Icon: MessageSquare, color: "text-gray-600 bg-gray-100",      subs: ["General feedback"] },
};

const CATEGORY_KEYS = Object.keys(CATEGORY_TREE);

// Fallback so legacy tickets (or unknown values) still render something
// recognisable instead of crashing the icon tile.
...
// Pill colours for legacy tickets that still have a non-"normal" priority
// from the old flow. The new wizard doesn't ask for priority, but the
// detail/list views still know how to render one if it shows up.
...
// Snapshot everything we can know about the browser/session at submit time.
// Runs silently — never shown in the UI — and gets attached to the ticket so
// admins can debug "what page / what browser" without asking.
...
export default function SupportPage() {
  ...
  const spName = session?.user?.RegName || session?.user?.name || "";
  const spEmail = session?.user?.email || "";
  const spType = (session?.user as { category?: string } | undefined)?.category || "";
  ...
  // Single-screen form. `submittedTicket` is the only gate — when it's set,
  // the form view is replaced by the confirmation screen.
  ...
  // reply state — text plus inline image attachments
  const [replyText, setReplyText] = useState("");
  const [replyAttachments, setReplyAttachments] = useState<File[]>([]);
  const [replying, setReplying] = useState(false);
  const replyFileInputRef = useRef<HTMLInputElement | null>(null);

  // lightbox state for tapping a screenshot to enlarge
  const [lightboxUrl, setLightboxUrl] = useState<string | null>(null);

  // Object URLs for inline preview of attached reply images. Recreated each
  // render — cleaned up in the effect below to avoid leaks.
  ...
  const fetchTickets = async () => {
    ...
    setLoading(true);
    try {
      // Cache-bust so the browser doesn't serve a stale list (admin replies
      // would otherwise stay invisible until a hard refresh).
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/support/tickets?submittedById=${spId}&_=${Date.now()}`,
        { cache: "no-store" }
      );
  ...
  // Always re-pull the latest ticket from the server when the user opens it,
  // otherwise admin replies that landed after the initial list-fetch are
  // missed and the SP appears to never see them.
  const openTicketFresh = async (t: Ticket) => {
    setOpenTicket(t); // show what we have immediately

    // If this ticket has the SP-unread flag, optimistically clear it locally
    // so the red dot disappears the moment the SP clicks the card.
    if (t.spUnread) {
      setTickets((prev) =>
        prev.map((x) => (x._id === t._id ? { ...x, spUnread: false } : x))
      );
      // Tell the server the SP has now seen this ticket. Best-effort.
      fetch(
    ...
    try {
      ...
      if (res.ok && json?.success && json?.data) {
        // Force the local copy to reflect the SP-seen state we just set.
  ...
  const closeWizard = () => {
    ...
    setShowForm(false);
    // Defer the reset so the modal animates out cleanly with the old content
  ...
  const handleCreate = async (e?: React.FormEvent) => {
    ...
    if (!fSubCategory) {
      toast({
        title: "Pick a sub-category",
        description: "Choose a section under " + CATEGORY_TREE[fCategory].label,
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      fd.append("subCategory", fSubCategory || "Other");
      // Priority dropped from the SP-facing wizard — every new ticket goes
      // in as "normal". Admins can still adjust on their side.
      fd.append("priority", "normal");
      fd.append("submittedById", spId);
      fd.append(
        "submittedBy",
        JSON.stringify({
          id: spId,
      ...
      fd.append("context", JSON.stringify(captureContext(spType)));
  ...
  const handleReply = async () => {
    if (!openTicket || !spId) return;
    // Must have either text or at least one image — matches the backend check.
    if (!replyText.trim() && replyAttachments.length === 0) return;
    setReplying(true);
    try {
      // Always multipart now since replies can carry image attachments. Empty
      // attachments[] is fine — the backend just sees no files.
      const fd = new FormData();
      fd.append("from", "sp");
      fd.append("fromId", spId);
      fd.append("fromName", spName);
      fd.append("text", replyText);
      // S3 key helper on the server needs submittedById to namespace uploads
      // under the right SP. Reuse the existing field used by createTicket.
      fd.append("submittedById", spId);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/30 dark:from-gray-950 dark:via-gray-950 dark:to-gray-900 px-4 py-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              Report a bug, request a feature, or ask the admin team a question.
            </p>
          </div>
          <button
            type="button"
      ...
      {/* ============================================================
          SINGLE-SCREEN TICKET FORM
          Row 1: Category chips      (Accounts / Content / Onboarding / …)
          Row 2: Sub-category chips  (auto-fills based on the chosen category)
          Row 3: Title + Description + Screenshots
          When `submittedTicket` is set, this view swaps to the confirmation.
      ============================================================ */}
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl ring-1 ring-black/5 w-full max-w-3xl max-h-[92vh] flex flex-col overflow-hidden">
                ...
                <div>
                  ...
                  {!submittedTicket && (
                    <p className="text-[11px] text-gray-500 dark:text-gray-400 leading-tight mt-0.5">
                      Tell us what went wrong — we&apos;ll route it to the right team.
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                <div className="px-6 py-5 space-y-5">
                  ...
                  <div>
                    <div className="flex items-baseline justify-between mb-2">
                      ...
                      {fCategory && (
                        <span className="text-[11px] text-gray-400">
                          Inside {CATEGORY_TREE[fCategory].label}
                        </span>
                      )}
                    </div>
                    {!fCategory ? (
                      <div className="px-4 py-3 rounded-xl border border-dashed border-gray-200 dark:border-gray-700 bg-gray-50/40 dark:bg-gray-800/20 text-[12px] text-gray-400 text-center">
                        Pick a category above to see its sub-categories
                      </div>
                    ) : (
                      <div className="flex flex-wrap gap-2">
                        {CATEGORY_TREE[fCategory].subs.map((s) => {
                          const active = fSubCategory === s;
                          return (
                            <button
                              type="button"
                              key={s}
                              onClick={() => setFSubCategory(s)}
                              className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full border text-[12.5px] font-medium transition ${
                                active
                                  ? "border-blue-500 bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300"
                                  : "border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:border-blue-300 hover:bg-blue-50/30"
                              }`}
                            >
              ...
              {/* Confirmation screen — swaps in after submission */}
              {submittedTicket && (
                <div className="px-6 py-12 text-center space-y-5">
                  ...
                  <div>
                    ...
                    <p className="mt-3 text-sm text-gray-500 dark:text-gray-400 max-w-sm mx-auto">
                      Our admin team will review your ticket and reply on this thread.
                    </p>
                  </div>
            ...
            {/* Footer — Cancel + Submit (hidden on confirmation screen) */}
            {!submittedTicket && (
              <div className="border-t border-gray-100 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-900/40 px-6 py-3 flex items-center justify-between gap-3">
                {/* Small contextual hint on the left so the footer doesn't
                    feel empty when stretched. */}
                <p className="hidden sm:block text-[11px] text-gray-400">
                  {fCategory && fSubCategory
                    ? `Filing under ${CATEGORY_TREE[fCategory].label} › ${fSubCategory}`
                    : "Pick a category and sub-category to continue"}
                </p>
      ...
      {/* Ticket detail drawer (full-screen modal) */}
      {openTicket && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div
            className="absolute inset-0 bg-black/40 backdrop-blur-sm"
          ...
          <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              ...
              {/* Auto-captured technical context — collapsed by default so
                  the SP knows we captured it but isn't distracted. */}
              ...
              {openTicket.thread?.length > 0 && (
                <div className="space-y-3">
                  {openTicket.thread.map((m, idx) => (
                    <div
                      key={m._id || idx}
                      className={`flex ${m.from === "sp" ? "justify-end" : "justify-start"}`}
                    >
                      <div
                        className={`max-w-[80%] rounded-lg px-3 py-2 ${m.from === "sp"
                            ? "bg-blue-600 text-white"
                            : "bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200"
                          }`}
                      >
                        <p className="text-[10px] font-semibold opacity-80 mb-0.5">
                          {m.from === "sp" ? "You" : `Admin · ${m.fromName}`}
                        </p>
                        {m.text && (
                          <p className="text-sm whitespace-pre-wrap">{m.text}</p>
                        )}
                        {/* Inline images sent with this message — tap to
