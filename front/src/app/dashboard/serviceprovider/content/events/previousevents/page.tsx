...
export default function PreviousEvents() {
  ...
  // Show only past events on this page (upcoming ones live in /upcomingevents).
  // Then narrow further by the selected date range, if any.
  const now = Date.now();
            ...
            return (
              ...
              >
                <div className="flex flex-col lg:flex-row">
                  {/* Banner — left, full bleed against the card edge */}
                  {eventImage ? (
                          ...
                          <div className="flex items-center gap-2 flex-wrap mb-1">
                            {eventType && (
                              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-[11px] font-semibold uppercase tracking-wide border border-blue-100 dark:border-blue-900/50">
                                {eventType === "Online" && <Globe className="w-3 h-3" />}
                                {eventType === "Offline" && <Building2 className="w-3 h-3" />}
                                {eventType === "Hybrid" && <Sparkles className="w-3 h-3" />}
                                {eventType}
                              </span>
                            )}
                            <span
